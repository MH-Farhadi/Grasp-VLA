"""
Pi-Zero v1 fine-tuning script (template).

This script is wired to this repo's `raw_data/` format and is intended to let you
swap in different VLA/VLM checkpoints over time.

Because "Pi-Zero" model releases can differ in architecture/API, this script uses
a **generic image+text -> text** supervised objective by default:
- Input: (image, instruction prompt)
- Target: a 7D action rendered as text: "dx dy dz drx dry drz gripper"

If your Pi-Zero checkpoint uses a different action representation (e.g., discretized
action tokens like OpenVLA, or continuous regression heads), this script is the
right place to adapt the collator/loss.
...
# This file lives under: train_scripts/pi-zero/pizero-v1.py
...
def format_action_text(action_7d: Sequence[float], precision: int = 4) -> str:
    ...
    fmt = f"{{:+.{precision}f}}"
    return " ".join([fmt.format(float(v)) for v in action_7d[:6]] + [str(int(round(float(action_7d[6]))))])
...
class TextVLADataCollator:
    ...
    def __call__(self, batch: Sequence[TextTargetSample]) -> Dict[str, torch.Tensor]:
        ...
        for i in range(len(batch)):
            ...
            full_ids = torch.cat([prompt_ids, tgt_ids, torch.tensor([eos], dtype=prompt_ids.dtype)])
...
def load_pizero_model_and_processor(
    model_id: str,
    *,
    ...
    If your Pi-Zero checkpoint needs `trust_remote_code=True`, this is enabled by default.
    """
    ...
    # Prefer the newer auto-class if available; fallback to Vision2Seq.
    try:
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Fine-tune Pi-Zero on ./raw_data (template)")
    p.add_argument("--model_id", type=str, required=True, help="HF model id or local path for Pi-Zero checkpoint")
    p.add_argument("--data_dir", type=str, default="raw_data", help="Path to raw_data directory")
    p.add_argument("--output_dir", type=str, default="checkpoints/pizero_v1", help="Where to save finetuned outputs")
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    print(f"Loading Pi-Zero model: {args.model_id}")
    model, processor = load_pizero_model_and_processor(
        args.model_id,
